...
BUILD_PIPELINE: tuple[PipelineStep, ...] = (
    ...
    PipelineStep(
        ...
        "qa",
        "Pre-build plain language — scripts non-developer friendly",
        when="pre_build",
        cli="validate-simple-language",
    ),
    PipelineStep(
        "validate-clip-trims",
        "qa",
        "Pre-build clip trim bounds + source cut suggestions",
        when="pre_build",
        cli="validate-clip-trims",
    ),
    PipelineStep(
        "validate-resource-usefulness",
        "qa",
        "Pre-build related resource topical usefulness",
        when="pre_build",
        cli="validate-resource-usefulness",
    ),
    PipelineStep(
...
PUBLISH_GATE: tuple[PipelineStep, ...] = (
    ...
    PipelineStep("validate-spoken-visual", "gate", "V4 spoken ↔ visual + visual claims", cli="validate-spoken-visual"),
    PipelineStep(
        "validate-visual-claims",
        "gate",
        "V4b on-screen table/chart claim vs pixels",
        cli="validate-visual-claims",
    ),
    ...
    PipelineStep(
        ...
        "gate",
        "V5d assembled clip duplicate check",
        cli="validate-visual-duplicates",
    ),
    PipelineStep(
        "validate-simple-language",
        "gate",
        "V5e non-developer plain language",
        cli="validate-simple-language",
    ),
    PipelineStep(
        "validate-clip-trims",
        "gate",
        "V5f clip trim range + cut suggestions",
        cli="validate-clip-trims",
    ),
    PipelineStep(
        "validate-resource-usefulness",
        "gate",
        "V5g related resource usefulness",
        cli="validate-resource-usefulness",
    ),
...
PYTEST_MODULES: tuple[str, ...] = (
    ...
    "tests/test_visual_duplicate_audit.py",
    "tests/test_simple_language_audit.py",
    "tests/test_clip_trim_audit.py",
    "tests/test_resource_usefulness_audit.py",
    "tests/test_visual_claim_audit.py",
